...
contract NFTMarketplace is ERC721URIStorage, Ownable {
    ...
    function listForSale(uint256 tokenId, uint256 price) public {
        ...
        require(
            ownerOf(tokenId) == msg.sender,
            "You must own this token to list it for sale"
        );
    ...
    function buy(uint256 tokenId) public payable {
        require(_exists(tokenId), "Please use an existing token");
        require(
            tokenIdToSeller[tokenId] != address(0),
            "Token is not listed for sale"
        );
        require(
            msg.value >= tokenIdToPrice[tokenId],
        ...
        payable(seller).transfer(price - ((price * 5) / 100)); // Deduct a 5% fee for the marketplace
